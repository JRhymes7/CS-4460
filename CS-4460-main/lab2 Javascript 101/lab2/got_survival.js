...
function updateBars(house) {
    ...
    for (i = 0; i < characters.length; i++) {
        console.log("hi");
        if (house == "top") {
            var power = characters.filter(function(person) {
                return person.power_ranking > 0;
            })
            power.sort((a,b) => (b.probability_of_survival - a.probability_of_survival));
            renderBars(power);
        } else {
            var fam = characters.filter(function(person) {
                return person.house == house;
        })
        fam.sort((a,b) => (b.probability_of_survival - a.probability_of_survival));
        renderBars(fam);
        }
    }
}
